...
export class BlockFeesGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-fees:See the average mining fees earned per HashCash block visualized in HCASH and USD over time.`);
  ...
  prepareChartOptions(data) {
    const feesBtcLabel = $localize`:@@graphs.blockFees.feesBtc:Fees HCASH`;
        ...
        formatter: function (data) {
          ...
          for (const tick of data) {
            if (tick.seriesIndex === 0) {
              tooltip += `${tick.marker} ${feesBtcLabel}: ${formatNumber(tick.data[1], this.locale, '1.3-3')} HCASH<br>`;
