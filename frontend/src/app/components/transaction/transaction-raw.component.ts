...
export class TransactionRawComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.preview-tx:Preview a transaction to the HashCash${seoDescriptionNetwork(this.stateService.network)} network using the transaction's raw hex data.`);
