...
export class BlockFeesSubsidyGraphComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription($localize`:@@meta.description.bitcoin.graphs.block-fees-subsidy:See the mining fees earned per HashCash block compared to the HashCash block subsidy, visualized in HCASH and USD over time.`);
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          for (let i = data.length - 1; i >= 0; i--) {
            ...
            if (this.displayMode === 'normal') {tooltip += `${formatNumber(tick.data, this.locale, '1.0-3')} HCASH<br>`;}
          ...
          if (this.displayMode === 'normal') {tooltip += `<div style="margin-left: 2px">${formatNumber(data.reduce((acc, val) => acc + val.data, 0), this.locale, '1.0-3')} HCASH</div>`;}
      ...
      yAxis: this.data.blockFees.length === 0 ? undefined : [
        {
          ...
          axisLabel: {
            color: 'var(--grey)',
            formatter: (val) => {
              return `${val}${this.displayMode === 'percentage' ? '%' : ' HCASH'}`;
            }
          },
          min: 0,
          max: (value) => {
