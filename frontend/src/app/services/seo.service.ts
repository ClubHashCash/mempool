...
export class SeoService {
  network = '';
  baseTitle = 'mempool';
  baseDescription = 'Explore the full HashCash ecosystem&reg; with The Mempool Open Source Project&reg;.';
  ...
  getTitle(): string {
    if (this.network === 'testnet')
      {return this.baseTitle + ' - HashCash Testnet3';}
    if (this.network === 'testnet4')
      {return this.baseTitle + ' - HashCash Testnet4';}
    if (this.network === 'signet')
      {return this.baseTitle + ' - HashCash Signet';}
    if (this.network === 'liquid')
      {return this.baseTitle + ' - Liquid Network';}
    if (this.network === 'liquidtestnet')
      {return this.baseTitle + ' - Liquid Testnet';}
    return this.baseTitle + ' - ' + (this.network ? this.ucfirst(this.network) : 'HashCash') + ' Explorer';
  }
